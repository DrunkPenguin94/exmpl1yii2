...
use common\models\ForumMessageSearch;
...
class ForumMessageController extends Controller
{
     ...
     * @return mixed
     */
    public function actionIndex($id_post)
    {
        $searchModel = new ForumMessageSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
        $dataProvider->query->where(["id_post"=>$id_post]);
        $dataProvider->query->orderby("date_create desc");

        $modelForumMessage=ForumFolder::find()->where(["id"=>$id_post])->one();
        return $this->render('index', [
            'searchModel' => $searchModel,
            'dataProvider' => $dataProvider,
            'modelForumMessage'=>$modelForumMessage
        ]);
    }
